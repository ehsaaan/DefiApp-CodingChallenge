...
contract standardToken is Context, IBEP20, IBEP20Metadata{
    ...
    constructor (address creator_,string memory name_, string memory symbol_,uint8 decimals_, uint256 tokenSupply_,
        ...
        state.mintflag = _canmint;
        state.burnflag = _canburn;
        state.pauseflag = _canpause;
        state.blacklistflag = _canBlacklist;
        
        _mint(creator_, tokenSupply_);
        
        setStatus(_canmint, _canburn, _canpause, _canBlacklist);
        
        _paused = false;
    }
    
    function setStatus(uint256 _canmint, uint256 _canburn, uint256 _canpause, uint256 _canBlacklist) internal virtual {
        state.mintflag = _canmint;
        state.burnflag = _canburn;
        state.pauseflag = _canpause;
        state.blacklistflag = _canBlacklist;
     ...
     */
    function mint(address account, uint256 amount) public virtual onlyOwner canMint {
    ...
    function blacklistUpdate(address user, bool value) public virtual onlyOwner canBlacklist {
        // require(_owner == _msgSender(), "Only owner is allowed to modify blacklist.");
        _blacklist[user] = value;
        emit BlacklistUpdated(user, value);
    }
    
    function isBlackListed(address user) public view virtual canBlacklist returns (bool) {
     ...
     */
    function _beforeTokenTransfer(address sender, address to, uint256 amount) internal virtual { 
        ...
        if( state.blacklistflag > 0 ) {
            require (!isBlackListed(sender), "Token transfer refused. Receiver is on blacklist");
...
contract LiquidityToken is Context, IBEP20 {
    ...
    constructor (address creator_, address unirouter, address reciever, uint8 decimal_, uint256 supply, string memory name_, string memory symbol_) {
        
        _name = name_;
        _symbol = symbol_;
        _decimals = decimal_;
        
        IPancakeRouter02 _PancakeRouter = IPancakeRouter02(unirouter);
    ...
    function set(address creator_, address reciever, uint256 supply) private {
    ...
    function setStatus(uint256 _canmint, uint256 _canburn, uint256 _canpause, uint256 _canBlacklist) external onlyManager {
        state.mintflag = _canmint;
        state.burnflag = _canburn;
        state.pauseflag = _canpause;
        state.blacklistflag = _canBlacklist;
    ...
    function setFee(uint256 settingflag, uint256[4] memory fee) external onlyManager {
    ...
    function mint(uint256 amount) public onlyOwner canMint {
        ...
        _tFeeTotal = _tFeeTotal.sub(amount);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressBNB(marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingDivisor));
...
contract CreateMange {
    ...
    function calcFee(uint256 _canmint, uint256 _canburn, uint256 _canpause, uint256 _canBlacklist) internal view returns(uint256) {
    ...
    function createStandard  (
        ...
        require(msg.value >= calcFee(_canmint, _canburn, _canpause, _canBlacklist), "Balance is insufficent");
    ...
    function createLiuidity(address creator_, address reciever, string memory name_, string memory symbol_, uint8 decimal_, uint256 supply, uint256 settingflag, uint256[4] memory fees, uint256 _canmint, uint256 _canburn, uint256 _canpause, uint256 _canBlacklist) public payable{
        
        require(msg.value >= calcFee(_canmint, _canburn, _canpause, _canBlacklist), "Balance is insufficent");
        
        LiquidityToken token = new LiquidityToken(creator_, router_address, reciever, decimal_, supply, name_, symbol_);
        token.setFee(settingflag, fees);
        token.setStatus(_canmint, _canburn, _canpause, _canBlacklist);
